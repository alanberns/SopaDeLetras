...
def main_comprobacion_palabra(palabra):
    ...
    if (tipoWiki != ""):
        ...
        layout = [[sg.Text('Ingrese la definicion de ' + palabra)],
                  [sg.InputText('')],
                  [sg.Ok()]]
        ventana_definicion = sg.Window('INGRESAR DEFINICION').Layout(layout)
        definicion = ""
        while definicion is "":
            definicion = ventana_definicion.Read()
...
def agregar_eliminar():
    ...
    while True:
        ...
        if button is not 'Terminar':
            if button == 'Agregar':
                window.Hide()
                sg.Popup('Por favor espere, puede demorar unos segundos')
        ...
        if num == 2 and ListaV:
            ...
            if ListaS:
                ...
                if ListaA:
                    ...
                    palabras.append(pal)
                    ListaA.remove(pal)
    return palabras, ListaSustantivos, ListaAdjetivos, ListaVerbos, str(cant_sustantivos), str(cant_adjetivos), str(
        cant_verbos)
...
def main():
    ...
    if bot is 'Ok':
        ...
        # funcion sopa
        Sopa(cant_sustantivos, cant_adjetivos, cant_verbos, color_sustantivos, color_adjetivos, color_verbos,
             orientacion,
